...
public class T01_promotion {
	...
	public static void main(String[] args) {
		...
		byte b1 = 20;  // 1byte
		short s1 = b1; // 1byte -> 2byte 
		
		int int1 = b1; // 1byte -> 4byte
		int1 = s1; // 2byte-> 4byte
		
		char ch1 = 'A'; //2byte 유니코드 A는 65
		int inch = ch1; //2byte -> 4byte 출력시 int로 형변환 돼서 유니코드 A는 65이므로 65가 출력됨
		System.out.println(ch1);
		System.out.println("'A'의 유니코드 : " + inch);
		
		double d1 = int1; //4byte -> 8byte
